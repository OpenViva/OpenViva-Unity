...
//MODIFIED to spit out zero alpha
...
Properties {
    ...
    _StarMap("StarMap", CUBE) = "" {}
    _NightBlend("Night Blend", Range(0, 1)) = 1.0
    _NightSky("Night Sky", Color) = (0, 0, 1, 1)
}
...
SubShader {
    ...
    Pass {
        ...
        uniform half _NightBlend;
        uniform half3 _NightSky;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        #else
            half horizonHeight = 1.+IN.skyGroundFactor;
        #endif

        
            horizonHeight *= horizonHeight;
            col += _NightSky*horizonHeight;
            half4 night = texCUBE(_StarMap, IN.skyView );
            col += night.rgb*tri( _Time.y*night.a )*_NightBlend;

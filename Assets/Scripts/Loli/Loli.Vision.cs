...
public partial class Loli : Character {
	...
	private void UpdateViewTimers(){
		...
		for( int i=viewItems.Count; i-->0; ){
			...
			if( viewItem.outOfViewTimer >= Mathf.Epsilon ){
				...
				if( viewItem.outOfViewTimer >= 2.0f ){	//forget after 2 seconds of out of view
					viewItems.objects.RemoveAt(i);
	...
	private void CheckNormalViewAwareness(){
		...
		}else{	//if not looking at anything, find most interesting in view
	...
	private float GetClampedPupilPitch( float normalizedPitch, float madEyelid ){	//pitch is div by 90.0f

...
public class WorldPointerRaycaster : MonoBehaviour{
    ...
    private void OnEnable(){
        var vrControls = VivaPlayer.user.controls as VRCharacterControls;
        if( !vrControls ){
        ...
        if( vrControls.rightHandedness ){
            pointer = vrControls.rightHandTPD.transform;
            clickAction = Viva.input.actions.vrControls.rightTriggerClick;
        }else{
            pointer = vrControls.leftHandTPD.transform;
            clickAction = Viva.input.actions.vrControls.leftTriggerClick;
        }
    ...
    private void Update(){
        ...
        if( pointer == null ){
            Ray camRay;
            if( Camera.main != null ){
                camRay = Camera.main.ScreenPointToRay( Viva.input.mousePosition );
            }else{
                camRay = new Ray();
            }

...
public class DynamicBoneColliderCapsule : DynamicBoneCollider
{
    ...
    {
        float radius = this.radius * Mathf.Abs(transform.lossyScale.x);
        float h = height * 0.5f - this.radius;
        if (h <= 0)
        {
            if (bound == Bound.Outside)
                OutsideSphere(ref particlePosition, particleRadius, transform.TransformPoint(center), radius);
            else
                InsideSphere(ref particlePosition, particleRadius, transform.TransformPoint(center), radius);
        }
        else
        {
            Vector3 c0 = center;
